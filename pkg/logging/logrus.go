...
func Setup() {
	...
	//设置日志级别
	logLevel, err := logrus.ParseLevel(setting.LogSetting.LogLevel)
	if err != nil {
	...
	enableEsPush := setting.LogSetting.EnableEsPush
	if enableEsPush {
		appName := setting.AppSetting.AppName + "_" + setting.ServerSetting.RunMode
		logstashHook, err := logstash.NewHookWithFields("udp", setting.LogSetting.EsPushURL, "", logrus.Fields{"appname": appName})
		if err != nil {
			logrus.Fatal(err)
		}
